...
int draw_graph(double slope, double y_intercept) {
  ...
  if (bounds[0] == bounds[1]) {
    x_axis_location = (HEIGHT / 2.00) - y_intercept;
    y_axis_location = (WIDTH / 2.00);
  } else {
    double x_axis_ratio = (0.0 - bounds[0]) / (bounds[1] - bounds[0]);
    x_axis_location = (int)((HEIGHT - 1) * x_axis_ratio);
    y_axis_location = WIDTH - 1;
  }
  if (x_axis_location < 0)
    x_axis_location = 0;
  if (x_axis_location > HEIGHT - 1)
  ...
  for (int i = 0; i < WIDTH; i++) {
    ...
    if (bounds[0] == bounds[1]) {
      ...
      normal_y =
          (int)((HEIGHT - 1) * ((y - bounds[0]) / (bounds[1] - bounds[0])));
    }
    if (normal_y < 0)
      normal_y = 0;
    if (normal_y > HEIGHT - 1)
      normal_y = HEIGHT - 1;
  ...
  for (int i = 0; i < HEIGHT; i++) {
    draw_symbol(y_axis_location, i, screen_buffer, '|');
  }
...
int main(int argc, char *argv[]) {
  ...
  double y_intercept;
  if (argc == 3) {
    slope = strtod(argv[1], NULL);
    y_intercept = strtof(argv[2], NULL);
  } else {
    if (argc != 1) {
      printf("Invalid arguments. Please input both the slope and the "
             "y-intercept, in that order.\n");
    }
    slope = ask_for_data("slope");
    y_intercept = ask_for_data("y_intercept");
  }
